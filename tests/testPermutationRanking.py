...
class TestPermutationRanking(unittest.TestCase):
	...
	def testReadInput(self):
		...
		self.assertEqual(output.getvalue(), "Rank of " + testWord[1] + " is: " + "2" + '\n')

		output2 = StringIO()
		readInput(testWord2, output=output2)
		self.assertEqual(output2.getvalue(), "Rank of " + testWord2[1] + " is: " + "1" + '\n')

		output3 = StringIO()
		readInput(testWord3, output=output3)
		self.assertEqual(output3.getvalue(), "Rank of " + testWord3[1] + " is: " + "4" + '\n')

		output4 = StringIO()
		readInput(testWord4, output=output4)
		self.assertEqual(output4.getvalue(), "Rank of " + testWord4[1] + " is: " + "24572" + '\n')

		output5 = StringIO()
		readInput(testWord5, output=output5)
		self.assertEqual(output5.getvalue(), "Rank of " + testWord5[1] + " is: " + "10743" + '\n')

		output6 = StringIO()
		readInput(testWordError, output=output6)
		self.assertEqual(output6.getvalue(), "Error: Word can only contain letters in the alphabet.\n")
	...
	def testCalculateRank(self):
		...
		self.assertEqual(calculateRank(testWord,precedingList,permutationsLost),2)
		self.assertEqual(calculateRank(testWord2,precedingList2,permutationsLost2),1)
		self.assertEqual(calculateRank(testWord3,precedingList3,permutationsLost3),4)
		self.assertEqual(calculateRank(testWord4,precedingList4,permutationsLost4),24572)
		self.assertEqual(calculateRank(testWord5,precedingList5,permutationsLost5),10743)
		self.assertEqual(calculateRank(testWord6,precedingList6,permutationsLost6),53032)
